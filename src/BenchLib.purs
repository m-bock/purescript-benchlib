...
newtype Bench out = Bench
  ( MayOnly
      { benchName :: String
      , run :: { size :: Size, iterations :: Int, reporters :: Array Reporter } -> Aff (SampleResult /\ out)
      }
  )
...
defaultBenchOpts :: forall ret. BenchOptsPure Size ret Unit
defaultBenchOpts =
...
benchImpl :: forall m inp ret out. Eq out => MonadBench m => String -> (BenchOptsPure Size ret Unit -> BenchOptsM m inp ret out) -> (inp -> m ret) -> Bench out
benchImpl benchName mkOpts benchFn = Bench $ notOnly
  { benchName
  , run: \{ size, iterations } -> do
...
checkResults :: forall out. Show out => Eq out => GroupOpts out -> Array Reporter -> ResultPerSize out -> Aff Unit
...
bench :: forall inp ret out. Eq out => String -> (BenchOptsPure Size ret Unit -> BenchOptsPure inp ret out) -> (inp -> ret) -> Bench out
...
benchM :: forall m inp ret out. MonadBench m => Eq out => String -> (BenchOptsM m Size ret Unit -> BenchOptsM m inp ret out) -> (inp -> m ret) -> Bench out
